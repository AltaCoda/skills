...
- **The git-backed path is deprecated — API and MCP authoring is the supported route.** Committing a `.sendql`, `.flow` or template file to a connected repository still works today, but SendOps decided on 2026-09-12 that managed authoring through the API, MCP and dashboard is primary and git authoring is being retired. Draft and save through the tools or the API; reach for the repo only when the user's own workflow already lives there, and say that it is the legacy route.
...
| Deploying templates or definitions **from their own repository** — "how do I deploy templates from my repo", rendering with React Email/MJML in CI, a pull-request preview of what a merge would change, keeping a diffable copy in git, moving off the git connection | **Push from CI** | `sendops push` (the `AltaCoda/sendops-push@v1` GitHub Action) posting a bundle to `POST /v1/import` — `dry_run: true` on pull requests, `false` on merge; `GET /v1/export` for the other direction · [`templates/react-email.md`](https://help.sendops.dev/templates/react-email.md) · API: [`bundle`](https://developers.sendops.dev/api-reference/bundle.md) |
