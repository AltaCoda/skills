...
description: >-
  ...
  authoring and deploying email templates, version history and restore, or
  pushing templates and definitions from their own CI. Trigger it even when the
  ...
  list", "why isn't this person in the drip", or "deploy my templates from my
  repo" all mean this skill should be consulted. It makes a complex product feel simple:
...
- **GitHub sync was retired on 2026-09-14.** Templates and definitions are authored in the dashboard, the Public API, MCP, or pushed from the customer's own CI with `sendops push`. There is no repository to connect, nothing is "git-backed" or "repo-owned" any more, and every template, segment, List, topic, attribute and workflow is editable by anyone holding the permission. If a user mentions connecting a repo, route them to **Push from CI** below — their files stay in their git, and CI posts a bundle to SendOps.
